    ...
    if UPLOAD_DIR: # 이미지 업로드 섹션
        ...
        if st.session_state.uploaded_image_paths:
            ...
            def delete_image_action(image_path_to_delete):
                ...
                if image_path_to_delete in st.session_state.uploaded_image_paths: st.session_state.uploaded_image_paths.remove(image_path_to_delete)
                st.session_state.image_uploader_key_counter += 1 
                st.rerun() 
            paths_to_display_and_delete = list(st.session_state.uploaded_image_paths) 
            ...
            elif not st.session_state.uploaded_image_paths: st.caption("업로드된 이미지가 없습니다.")
            elif paths_to_display_and_delete: st.caption("표시할 유효한 이미지가 없습니다.")
    else:
